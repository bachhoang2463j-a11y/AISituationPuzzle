...
3. `<Puzzle>` 必须有 `游戏数据栏`，其中必须有一个 `主持人`；`玩家N` 键可以为 0 个到多个（支持 0 伙伴的单人推理局），存在时必须从 `玩家1` 连续编号。
...
> 分轮施工顺序以 [TurtleSoup_DEVELOPMENT_PLAN.md](./TurtleSoup_DEVELOPMENT_PLAN.md) 为准（视觉基线优先，吸取旧实现废弃的教训）；本节保留为能力依赖的参考顺序。
